...
def edicion_objetivo(ahora):
    ...
    La ventana solo se extiende HACIA ADELANTE desde la hora objetivo (nunca
    hacia atrás), para no pisar la ventana de la edición anterior. Esto
    absorbe atrasos del cron de GitHub Actions sin necesidad de tocar el
    workflow.
    """
...
# Estilo visual (gradiente + icono) por condición climática, agrupado por categoría.
# "oscuro" define si el texto de la tarjeta debe ser oscuro (fondos muy claros) o blanco.
WMO_ESTILOS = {
...
def estilo_clima(codigo):
    """Devuelve {icono, gradiente, oscuro} para un código WMO, con 'nublado' como default."""
    for estilo in WMO_ESTILOS.values():
        if codigo in estilo["codigos"]:
            return estilo
    return WMO_ESTILOS["nublado"]

# Íconos por métrica de la barra superior (dólar, UF, UV, aire)
METRICA_ICONOS = {
...
# Ícono y color por categoría de noticia — las 3 localidades comparten color
# (son la identidad "local" del boletín); el resto tiene un color propio.
CATEGORIA_ESTILO = {
...
CSS_MANANA = """
  ...
  /* CLIMA CARDS — fondo degradado según condición, con ícono grande de marca de agua */
  ...
  /* NOTICIAS OPTIMIZADAS (MAÑANA) — color por categoría vía estilo inline */
...
CSS_NOCHE = """
  ...
  /* NOTICIAS OPTIMIZADAS (NOCHE) — color por categoría vía estilo inline */
  .noticias-cols { display: flex; gap: 18px; margin-top: 4px; }
  .col { flex: 1; display: flex; flex-direction: column; gap: 12px; }
  .subgrupo { margin-bottom: 0; }
  .subgrupo-titulo { display: inline-flex; align-items: center; gap: 5px; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.6px; padding: 4px 10px; border-radius: 20px; margin-bottom: 8px; }
...
def renderizar_plantilla_html(ahora, fecha_reporte, es_manana, eco, feriado, luna, extra, datos_clima, datos_noticias, datos_combustible, html_futbol=""):
    ...
    # Clima Cards — cada tarjeta usa un degradado e ícono según la condición del momento
    clima_cards_html = ""
    uv_max_prom = 0
    ...
    # Noticias en dos columnas (5 categorías por lado) — color/ícono propios por categoría
    ...
    # Combustibles HTML — tinte de fondo suave con el mismo color del badge de cada tipo
...
async def main_async():
    ...
    if edicion is not None:
        ...
        es_manana = ahora.hour < 15  # fallback para ejecuciones manuales forzadas
