...
combustibles con badges de colores integrados, 10 categorías de noticias con bajadas en 2 columnas, tabla de fútbol, frase/datos del Maule y Noticia Destacada del Día.
"""
...
# Ventana de tolerancia en minutos alrededor del objetivo (ej. ±30 minutos)
...
def renderizar_plantilla_html(ahora, fecha_reporte, es_manana, eco, feriado, luna, extra, datos_clima, datos_noticias, datos_combustible, html_futbol="", articulo=None):
    ...
    # HTML para la Noticia Destacada
    html_destacada = ""
...
async def main_async():
    ...
    # 3. Noticias (con control de duplicados)
    global TITULOS_VISTOS
    TITULOS_VISTOS = set()

    datos_noticias = {}
    
    # 3.1 Noticia Destacada primero
    destacada = buscar_noticias("noticias Maule Chile", 1)
    if not destacada:
        destacada = noticias_tema("NATION", 1)
    datos_noticias["Destacada"] = destacada

    # 3.2 Secciones estándar (filtrarán automáticamente la destacada si coincide)
    datos_noticias["Longaví"] = buscar_noticias('"Longaví" Chile', 2)
    datos_noticias["Linares"] = buscar_noticias('"Linares" Chile', 2)
    datos_noticias["Yerbas Buenas"] = buscar_noticias('"Yerbas Buenas" Chile', 2)
    datos_noticias["Chile"] = noticias_tema("NATION", 2)
    datos_noticias["Mundo"] = noticias_tema("WORLD", 2)
    datos_noticias["Salud"] = buscar_noticias("salud bienestar Chile", 2)
    datos_noticias["Tecnología"] = noticias_tema("TECHNOLOGY", 2)
    datos_noticias["Deportes"] = noticias_tema("SPORTS", 2)
    datos_noticias["Entretención"] = buscar_noticias("espectaculos entretenimiento Chile", 2)
    datos_noticias["Tendencias"] = buscar_noticias("tendencias viral Chile", 2)
