...
combustibles con badges de colores integrados, 10 categorías de noticias con bajadas en 2 columnas,
tabla de fútbol, imagen del día y frases/datos del Maule.
Tipografía y captura optimizadas en HiDPI (Scale 3) para máxima nitidez en Telegram.
"""
...
def obtener_imagen_del_dia():
    try:
        ...
        if r.ok:
            ...
            if data.get("media_type") == "image":
                titulo = data.get("title", "Imagen Astronómica del Día")
                url_img = data.get("url") or data.get("hdurl")
                explicacion = data.get("explanation", "")
                
                if len(explicacion) > 130:
                    explicacion = explicacion[:127] + "…"
                    
                return {
                    "url": url_img,
                    "titulo": titulo,
                    "desc": explicacion or "Fotografía destacada del universo por la NASA."
                }
    except Exception:
        pass
    ...
    return {
        ...
        "titulo": "Paisaje de la Naturaleza",
        "desc": "Captura destacada de la flora y paisajes naturales."
    }
...
def renderizar_tabla_futbol(tabla):
    if not tabla:
        ...
        <div class="futbol-box">
          ...
          <div style="font-size:12px;color:#94A3B8;margin-top:8px;text-align:center;padding:10px;">
...
# ==========================================
# ESTILOS CSS CON TAMAÑOS OPTIMIZADOS
# ==========================================

CSS_MANANA = """
  @import url('https://rsms.me/inter/inter.css');
  body { font-family: 'Inter', -apple-system, BlinkMacSystemFont, sans-serif; color: #1E293B; background: #FFFFFF; width: 794px; font-size: 13px; line-height: 1.45; -webkit-font-smoothing: antialiased; }
  ...
  .eyebrow { font-size: 11.5px; letter-spacing: 1.2px; text-transform: uppercase; color: #1D63ED; font-weight: 700; background: #EBF3FE; padding: 5px 12px; border-radius: 6px; }
  .titulo { font-weight: 800; font-size: 44px; color: #1E293B; letter-spacing: -1px; margin-top: 10px; }
  .titulo .acento { color: #1D63ED; }
  .fecha-box { text-align: right; color: #64748B; font-size: 13.5px; line-height: 1.5; }
  .fecha-box .dia { font-size: 15.5px; color: #1E293B; font-weight: 600; }
  ...
  .comunas { margin-top: 14px; color: #64748B; font-size: 13.5px; letter-spacing: 0.2px; }
  ...
  .metric-item { font-size: 12px; color: #64748B; font-weight: 600; }
  .metric-value { font-weight: 800; font-size: 16px; color: #1D63ED; margin-top: 3px; }
  ...
  .holiday-title { font-size: 11.5px; font-weight: 700; color: #1D63ED; text-transform: uppercase; letter-spacing: 0.5px; }
  .holiday-desc { font-size: 13.5px; font-weight: 700; color: #1E293B; margin-top: 2px; }
  .holiday-badge { background: #1D63ED; color: #FFFFFF; font-weight: 700; font-size: 11.5px; padding: 5px 12px; border-radius: 20px; }
  ...
  .clima-card .ciudad { font-weight: 700; font-size: 18px; color: #0F172A; }
  .clima-card .desc { font-size: 13px; color: #475569; margin-top: 2px; font-weight: 500; }
  ...
  .clima-card .minmax { font-size: 13px; color: #475569; font-weight: 600; }
  .clima-card .viento { font-size: 12.5px; color: #334155; font-weight: 600; margin-top: 4px; }
  ...
  .gauge-titulo { font-weight: 700; font-size: 15.5px; color: #DC2626; letter-spacing: -0.2px; }
  .gauge-nota { font-size: 12px; color: #64748B; font-weight: 500; }
  ...
  .gauge-cero-label { position: absolute; top: -28px; transform: translateX(-50%); font-size: 11px; font-weight: 700; color: #334155; background: #FFFFFF; padding: 2px 6px; border-radius: 4px; border: 1px solid #CBD5E1; }
  ...
  .gauge-punto .etiqueta { position: absolute; left: 50%; font-size: 11px; white-space: nowrap; padding: 4px 8px; border-radius: 6px; display: flex; gap: 4px; align-items: center; box-shadow: 0 2px 6px rgba(0,0,0,0.06); background: #FFFFFF; color: #0F172A; border: 1px solid #E2E8F0; }
  ...
  .gauge-escala { display: flex; justify-content: space-between; margin-top: 35px; font-size: 11px; color: #64748B; font-weight: 600; }

  /* FASE LUNAR */
  .lunar-bar { background: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 12px; padding: 14px; margin-top: 16px; }
  .lunar-title { font-size: 11.5px; text-transform: uppercase; letter-spacing: 1px; color: #64748B; font-weight: 700; margin-bottom: 10px; text-align: center; }
  .lunar-days { display: flex; justify-content: space-around; align-items: center; }
  .lunar-day { display: flex; flex-direction: column; align-items: center; font-size: 11px; color: #64748B; font-weight: 500; }
  .lunar-day.today { color: #1D63ED; font-weight: 700; transform: scale(1.1); }
  .lunar-icon { font-size: 20px; margin: 2px 0; }

  /* NOTICIAS */
  .noticias-cols { display: flex; gap: 24px; margin-top: 4px; }
  .col { flex: 1; }
  .subgrupo { margin-bottom: 12px; }
  .subgrupo-titulo { display: inline-block; font-size: 11px; font-weight: 700; color: #1D63ED; text-transform: uppercase; letter-spacing: 0.8px; background: #EBF3FE; padding: 3px 8px; border-radius: 4px; margin-bottom: 6px; }
  .noticia { font-size: 13px; line-height: 1.45; margin-bottom: 10px; padding: 10px 12px; background: #F8FAFC; border-left: 3px solid #1D63ED; border-radius: 0 8px 8px 0; border: 1px solid #E2E8F0; border-left-width: 3px; }
  .noticia .titulo-n { color: #0F172A; font-weight: 700; font-size: 13px; letter-spacing: -0.1px; line-height: 1.3; }
  .noticia .bajada-n { color: #334155; font-size: 12px; margin-top: 4px; font-weight: 400; line-height: 1.4; }
  .noticia .fuente-n { color: #64748B; font-size: 11px; margin-top: 4px; font-weight: 600; }

  /* TABLA FÚTBOL */
  .futbol-box { margin-top: 16px; background: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 12px; padding: 14px 16px; }
  .futbol-titulo { font-size: 14px; font-weight: 700; color: #1E293B; }
  .futbol-subtitulo { font-size: 12px; color: #64748B; font-weight: 500; }
  .futbol-tabla { width: 100%; border-collapse: collapse; table-layout: fixed; }
  .futbol-tabla th { font-size: 10px; color: #64748B; text-transform: uppercase; letter-spacing: 0.4px; font-weight: 700; padding: 0 0 6px 0; text-align: center; }
  .futbol-tabla th.izq { text-align: left; }
  .futbol-tabla td { padding: 6px 0; font-size: 12px; color: #334155; border-top: 1px solid #E2E8F0; text-align: center; }
  .futbol-pos { font-weight: 700; color: #64748B; text-align: left !important; }
  .futbol-escudo { width: 18px; height: 18px; object-fit: contain; }
  .futbol-equipo { font-size: 12.5px; font-weight: 600; color: #0F172A; text-align: left !important; }
  .futbol-pts { font-size: 13px; font-weight: 800; color: #1D63ED; }
  ...
  /* IMAGEN DEL DÍA */
  .apod-box { margin-top: 16px; background: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 12px; padding: 14px; display: flex; gap: 14px; align-items: center; }
  .apod-img { width: 110px; height: 110px; border-radius: 8px; object-fit: cover; flex-shrink: 0; border: 1px solid #CBD5E1; }
  .apod-content { flex: 1; }
  .apod-tag { font-size: 10.5px; font-weight: 700; color: #1D63ED; text-transform: uppercase; letter-spacing: 0.8px; background: #EBF3FE; padding: 2px 8px; border-radius: 4px; display: inline-block; margin-bottom: 4px; }
  .apod-title { font-size: 14px; font-weight: 700; color: #0F172A; line-height: 1.25; margin-bottom: 4px; }
  .apod-desc { font-size: 12px; color: #334155; line-height: 1.4; font-weight: 400; }

  /* COMBUSTIBLES */
  .combustible-box { margin-top: 16px; background: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 12px; padding: 20px 22px; }
  .combustible-titulo { color: #0F172A; font-size: 15.5px; font-weight: 700; display: flex; justify-content: space-between; align-items: center; }
  .combustible-titulo span.nota { font-size: 12px; color: #64748B; font-weight: 500; }
  ...
  .precio-badge { display: inline-block; font-size: 11.5px; font-weight: 700; padding: 3px 10px; border-radius: 6px; margin-bottom: 6px; }
  .precio-card .monto { font-size: 22px; color: #0F172A; font-weight: 800; margin-top: 2px; letter-spacing: -0.5px; }
  .precio-card .ciudad-p { font-size: 12px; color: #1E293B; font-weight: 600; margin-top: 4px; }
  .precio-card .direccion-p { font-size: 10.5px; color: #64748B; margin-top: 3px; line-height: 1.3; min-height: 24px; display: flex; align-items: center; justify-content: center; }
  ...
  .footer-box-title { font-size: 11.5px; font-weight: 700; color: #64748B; margin-bottom: 4px; text-transform: uppercase; }
  .footer-box-text { font-size: 12.5px; color: #1E293B; font-style: italic; line-height: 1.45; }

  .footer { margin: 28px 48px 0 48px; padding-top: 16px; border-top: 1px solid #E2E8F0; display: flex; justify-content: space-between; font-size: 12px; color: #64748B; font-weight: 500; }
"""

CSS_NOCHE = """
  @import url('https://rsms.me/inter/inter.css');
  body { font-family: 'Inter', -apple-system, BlinkMacSystemFont, sans-serif; color: #F8FAFC; background: #0F172A; width: 794px; font-size: 13px; line-height: 1.45; -webkit-font-smoothing: antialiased; }
  .pagina { padding: 0 0 32px 0; background: #0F172A; }
  
  .masthead { padding: 40px 48px 22px 48px; }
  .masthead-top { display: flex; justify-content: space-between; align-items: baseline; }
  .eyebrow { font-size: 11.5px; letter-spacing: 1.2px; text-transform: uppercase; color: #38BDF8; font-weight: 700; background: #1E293B; padding: 5px 12px; border-radius: 6px; border: 1px solid #334155; }
  .titulo { font-weight: 800; font-size: 44px; color: #FFFFFF; letter-spacing: -1px; margin-top: 10px; }
  .titulo .acento { color: #38BDF8; }
  .fecha-box { text-align: right; color: #94A3B8; font-size: 13.5px; line-height: 1.5; }
  .fecha-box .dia { font-size: 15.5px; color: #F8FAFC; font-weight: 600; }
  ...
  .comunas { margin-top: 14px; color: #94A3B8; font-size: 13.5px; letter-spacing: 0.2px; }
  ...
  .metric-item { font-size: 12px; color: #94A3B8; font-weight: 600; }
  .metric-value { font-weight: 800; font-size: 16px; color: #38BDF8; margin-top: 3px; }
  ...
  .holiday-title { font-size: 11.5px; font-weight: 700; color: #38BDF8; text-transform: uppercase; letter-spacing: 0.5px; }
  .holiday-desc { font-size: 13.5px; font-weight: 700; color: #F8FAFC; margin-top: 2px; }
  .holiday-badge { background: #38BDF8; color: #0F172A; font-weight: 700; font-size: 11.5px; padding: 5px 12px; border-radius: 20px; }
  ...
  .clima-card .ciudad { font-weight: 700; font-size: 18px; color: #FFFFFF; }
  .clima-card .desc { font-size: 13px; color: #CBD5E1; margin-top: 2px; font-weight: 500; }
  .clima-fila-datos { display: flex; align-items: flex-end; justify-content: space-between; margin-top: 16px; }
  .clima-card .temp { font-size: 42px; font-weight: 800; color: #38BDF8; line-height: 0.9; letter-spacing: -1px; }
  .clima-card .minmax { font-size: 13px; color: #CBD5E1; font-weight: 600; }
  .clima-card .viento { font-size: 12.5px; color: #CBD5E1; font-weight: 600; margin-top: 4px; }

  /* GAUGE HELADAS */
  .gauge-box { margin-top: 16px; background: #1E293B; border: 1px solid #334155; border-radius: 12px; padding: 35px 26px; }
  .gauge-header { display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 30px; }
  .gauge-titulo { font-weight: 700; font-size: 15.5px; color: #F87171; letter-spacing: -0.2px; }
  .gauge-nota { font-size: 12px; color: #94A3B8; font-weight: 500; }
  ...
  .gauge-cero-label { position: absolute; top: -28px; transform: translateX(-50%); font-size: 11px; font-weight: 700; color: #FFFFFF; background: #0F172A; padding: 2px 6px; border-radius: 4px; border: 1px solid #334155; }
  ...
  .gauge-punto .etiqueta { position: absolute; left: 50%; font-size: 11px; white-space: nowrap; padding: 4px 8px; border-radius: 6px; display: flex; gap: 4px; align-items: center; box-shadow: 0 2px 6px rgba(0,0,0,0.3); background: #0F172A; color: #F1F5F9; border: 1px solid #334155; }
  .gauge-punto.pos-abajo .etiqueta { top: 25px; }
  .gauge-punto.pos-arriba .etiqueta { bottom: 25px; }
  .gauge-punto .etiqueta b { font-weight: 700; }
  .gauge-punto.alerta .etiqueta b { color: #F87171; }
  .gauge-escala { display: flex; justify-content: space-between; margin-top: 35px; font-size: 11px; color: #94A3B8; font-weight: 600; }

  /* FASE LUNAR */
  .lunar-bar { background: #1E293B; border: 1px solid #334155; border-radius: 12px; padding: 14px; margin-top: 16px; }
  .lunar-title { font-size: 11.5px; text-transform: uppercase; letter-spacing: 1px; color: #94A3B8; font-weight: 700; margin-bottom: 10px; text-align: center; }
  .lunar-days { display: flex; justify-content: space-around; align-items: center; }
  .lunar-day { display: flex; flex-direction: column; align-items: center; font-size: 11px; color: #94A3B8; font-weight: 500; }
  ...
  .col { flex: 1; }
  .subgrupo { margin-bottom: 12px; }
  .subgrupo-titulo { display: inline-block; font-size: 11px; font-weight: 700; color: #38BDF8; text-transform: uppercase; letter-spacing: 0.8px; background: #1E293B; padding: 3px 8px; border-radius: 4px; margin-bottom: 6px; border: 1px solid #334155; }
  .noticia { font-size: 13px; line-height: 1.45; margin-bottom: 10px; padding: 10px 12px; background: #1E293B; border-left: 3px solid #38BDF8; border-radius: 0 8px 8px 0; border: 1px solid #334155; border-left-width: 3px; }
  .noticia .titulo-n { color: #FFFFFF; font-weight: 700; font-size: 13px; letter-spacing: -0.1px; line-height: 1.3; }
  .noticia .bajada-n { color: #CBD5E1; font-size: 12px; margin-top: 4px; font-weight: 400; line-height: 1.4; }
  .noticia .fuente-n { color: #94A3B8; font-size: 11px; margin-top: 4px; font-weight: 600; }

  /* TABLA FÚTBOL */
  .futbol-box { margin-top: 16px; background: #1E293B; border: 1px solid #334155; border-radius: 12px; padding: 14px 16px; }
  .futbol-titulo { font-size: 14px; font-weight: 700; color: #F8FAFC; }
  .futbol-subtitulo { font-size: 12px; color: #94A3B8; font-weight: 500; }
  .futbol-tabla { width: 100%; border-collapse: collapse; table-layout: fixed; }
  .futbol-tabla th { font-size: 10px; color: #94A3B8; text-transform: uppercase; letter-spacing: 0.4px; font-weight: 700; padding: 0 0 6px 0; text-align: center; }
  .futbol-tabla th.izq { text-align: left; }
  .futbol-tabla td { padding: 6px 0; font-size: 12px; color: #E2E8F0; border-top: 1px solid #334155; text-align: center; }
  .futbol-pos { font-weight: 700; color: #94A3B8; text-align: left !important; }
  .futbol-escudo { width: 18px; height: 18px; object-fit: contain; }
  .futbol-equipo { font-size: 12.5px; font-weight: 600; color: #FFFFFF; text-align: left !important; }
  .futbol-pts { font-size: 13px; font-weight: 800; color: #38BDF8; }
  ...
  /* IMAGEN DEL DÍA */
  .apod-box { margin-top: 16px; background: #1E293B; border: 1px solid #334155; border-radius: 12px; padding: 14px; display: flex; gap: 14px; align-items: center; }
  .apod-img { width: 110px; height: 110px; border-radius: 8px; object-fit: cover; flex-shrink: 0; border: 1px solid #334155; }
  .apod-content { flex: 1; }
  .apod-tag { font-size: 10.5px; font-weight: 700; color: #38BDF8; text-transform: uppercase; letter-spacing: 0.8px; background: #0F172A; padding: 2px 8px; border-radius: 4px; display: inline-block; margin-bottom: 4px; border: 1px solid #334155; }
  .apod-title { font-size: 14px; font-weight: 700; color: #FFFFFF; line-height: 1.25; margin-bottom: 4px; }
  .apod-desc { font-size: 12px; color: #CBD5E1; line-height: 1.4; font-weight: 400; }

  /* COMBUSTIBLES */
  .combustible-box { margin-top: 16px; background: #1E293B; border: 1px solid #334155; border-radius: 12px; padding: 20px 22px; }
  .combustible-titulo { color: #FFFFFF; font-size: 15.5px; font-weight: 700; display: flex; justify-content: space-between; align-items: center; }
  .combustible-titulo span.nota { font-size: 12px; color: #94A3B8; font-weight: 500; }
  .precios-fila { display: flex; gap: 12px; margin-top: 14px; }
  ...
  .precio-card .ciudad-p { font-size: 12px; color: #F1F5F9; font-weight: 600; margin-top: 4px; }
  .precio-card .direccion-p { font-size: 10.5px; color: #94A3B8; margin-top: 3px; line-height: 1.3; min-height: 24px; display: flex; align-items: center; justify-content: center; }
  ...
  .footer-box-title { font-size: 11.5px; font-weight: 700; color: #94A3B8; margin-bottom: 4px; text-transform: uppercase; }
  .footer-box-text { font-size: 12.5px; color: #F8FAFC; font-style: italic; line-height: 1.45; }

  .footer { margin: 28px 48px 0 48px; padding-top: 16px; border-top: 1px solid #1E293B; display: flex; justify-content: space-between; font-size: 12px; color: #94A3B8; font-weight: 500; }
"""

# ==========================================
# RENDERIZADO HTML Y PLAYWRIGHT (HiDPI 3X)
# ==========================================
...
async def html_a_imagen(html_content: str) -> bytes:
    async with async_playwright() as p:
        ...
        # device_scale_factor=3 forzado para nitidez HiDPI / Retina (2382px de ancho real)
        page = await browser.new_page(
            viewport={"width": 794, "height": 1000}, 
...
def renderizar_plantilla_html(ahora, fecha_reporte, es_manana, eco, feriado, luna, extra, datos_clima, datos_noticias, datos_combustible, html_futbol="", datos_apod=None):
    ...
    if datos_apod:
        html_apod = f"""
        <div class="apod-box">
          <img class="apod-img" src="{datos_apod['url']}" alt="Imagen del día">
          <div class="apod-content">
            <span class="apod-tag">📷 Imagen del Día</span>
            <div class="apod-title">{html.escape(datos_apod['titulo'])}</div>
            <div class="apod-desc">{html.escape(datos_apod['desc'])}</div>
          </div>
        </div>"""
...
def enviar_foto_telegram(imagen_bytes, caption_text=""):
    ...
    try:
        ...
        print("Boletín enviado con éxito a Telegram (HiDPI 3X).")
...
async def main_async():
    ...
    # 1. Datos económicos, feriados, cultura e imagen del día
    ...
    datos_apod = obtener_imagen_del_dia()
    ...
    # 6. Renderizado PNG en Alta Definición (3X)
    print("🎨 Generando HTML y renderizando PNG HiDPI a escala 3x con Playwright...")
    html_final = renderizar_plantilla_html(
        ahora, fecha_reporte, es_manana, eco, feriado, luna, extra, 
        datos_clima, datos_noticias, datos_combustible, html_futbol, datos_apod
    )
    ...
    # 8. Enviar
    print("📤 Enviando foto en alta definición a Telegram...")
